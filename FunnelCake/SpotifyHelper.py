...
def clone(manager: PlaylistManager, src: str, force_override=False, custom_name=None):
    if not isinstance(manager, PlaylistManager.PlaylistManager) or not isinstance(
        src, str
    ):
        raise ValueError

    src = SpotifyPlaylist.SpotifyPlaylist.from_url(manager, src)
    destination_url = manager.create(
        f"{src.name} | CLONED" if not custom_name else custom_name
    )
    if isinstance(destination_url, bool) or force_override:
        print(
            f"[ERROR] Cannot clone {src.name}, it already exists (no override specified)"
        )
        return
...
def merge(container: list, manager: PlaylistManager, output_name: str):
    ...
    destination_url = manager.create(
        output_name,
        True,
        f"Playlist from {len(container)} playlists: {names}. Collaborators(s): {collabs}",
    )
    new_playlist = SpotifyPlaylist.SpotifyPlaylist.from_url(manager, destination_url)

    if isinstance(destination_url, bool):
        print(
            f'[ERROR] Could not store contents of {len(container)} playlists @ "{output_name}"; already exists'
        )
        return
    else:
...
def analyze(playlist: SpotifyPlaylist, manager: PlaylistManager) -> dict:
    ...
    content = [
        subelement
        for element in playlist.get_detailed_track_info()
